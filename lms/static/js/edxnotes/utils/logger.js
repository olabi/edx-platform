...
define(['underscore', 'logger'], function (_, Logger) {
     ...
     * @param  {Boolean} async     Indicates whether request is an asynchronous.
     */
    NotesLogger.prototype.emit = function (eventName, data, requestOptions) {
        // `datetime` is a common for all events.
        if (_.isArray(data)) {
            ...
            data = _.extend({datetime: new Date()}, data);
        }

        this.log(eventName, data);
        if (requestOptions) {
            Logger.log(eventName, data, null, requestOptions);
        } else {
