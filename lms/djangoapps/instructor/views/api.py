...
def generate_registration_codes(request, course_id):
    ...
    """
    course_id = locator.CourseLocator(course_id)
    invoice_copy = False
    ...
    if len(paid_modes) != 1:
        ...
        log.error(msg)
        return HttpResponse(status=500, content=_(msg))
    ...
    for _ in range(course_code_number):  # pylint: disable=redefined-outer-name
        generated_registration_code = save_registration_code(
            request.user, course_id, course_mode.mode_slug, sale_invoice, order=None
        )
